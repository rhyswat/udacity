...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        # Add code here
        #
        # Want cte > 0 when the car is outside the track, <0 when within.
...
def run(params, radius, printflag = False, plot=False):
    plot_x = []
    plot_y = []
    ...
    for i in range(N*2):
        if plot :
            plot_x.append(myrobot.x)
            plot_y.append(myrobot.y)
    ...
    if plot :
        return (err / float(N), plot_x, plot_y)
    return err / float(N)

radius = 25.0
params = [10.0, 15.0, 0]
err, x, y = run(params, radius, printflag=False, plot=True)
