...
# --------
#
# dot:  the dot product of u and v
#

def dot(u, v) :
    return float(sum(u[i]*v[i] for i in xrange(len(u))))
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        ### ENTER CODE HERE
        assert index < len(spath)
        p1 = spath[index]
        p2 = spath[index + 1]
        delta = [p2[j] - p1[j] for j in xrange(len(p1))]
        d2 = dot(delta, delta)
        r = [estimate[j] - p1[j] for j in xrange(len(p1))]
        n = (-delta[1], delta[0])
        u = dot(r, delta)/d2
        cte = dot(n, r)/d2
        if u > 1 :
